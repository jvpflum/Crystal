...
export function SubagentsView() {
  ...
  const sendAgentMessage = useCallback(async (message: string): Promise<{ stdout: string; code: number }> => {
    const result = await invoke<{ stdout: string; stderr: string; code: number }>("execute_command", {
      command: `openclaw agent --agent main --message "${escapeMsg(message)}" --json`,
      cwd: null,
    });
  ...
  const loadSubagents = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await sendAgentMessage("/subagents list");
      if (result.code === 0 && result.stdout.trim()) {
        try {
          const data = JSON.parse(result.stdout);
          const list: SubAgent[] = Array.isArray(data)
            ? data
            : (data.subagents ?? data.agents ?? data.items ?? data.result ?? []);
          if (Array.isArray(list)) {
            setSubagents(list);
          } else {
            setSubagents([]);
            setOutput({ title: "Sub-Agents List", content: result.stdout });
          }
        } catch {
          setSubagents([]);
          setOutput({ title: "Sub-Agents List", content: result.stdout });
      ...
      setError(e instanceof Error ? e.message : "Failed to list sub-agents");
    }
    setLoading(false);
  }, [sendAgentMessage]);
  ...
  useEffect(() => { loadSubagents(); loadAgents(); loadModels(); }, [loadSubagents, loadAgents, loadModels]);
  ...
  const subagentAction = async (label: string, message: string) => {
    ...
    try {
      ...
      if (result.code === 0) {
        ...
        if (result.stdout.trim()) {
          setOutput({ title: label, content: result.stdout });
        }
        if (label.startsWith("Kill")) await loadSubagents();
      } else {
  ...
  const statusColor = (s?: string) => {
    ...
    if (l === "killed" || l === "stopped") return "#fbbf24";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <p style={{ margin: "2px 0 0", fontSize: 10, color: "var(--text-muted)" }}>
                ...
                {subagents.filter(s => s.status?.toLowerCase() === "running").length > 0 && (
                  <> &middot; <span style={{ color: "#4ade80" }}>{subagents.filter(s => s.status?.toLowerCase() === "running").length} running</span></>
                )}
              </p>
            </div>
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            <button
              ...
              disabled={isLoading("Kill All") || subagents.length === 0}
              style={{ ...BTN_DANGER, opacity: subagents.length === 0 ? 0.4 : 1 }}
            >
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 20px" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <button
            ...
            style={{
              ...
              background: "var(--bg-elevated)", border: "1px solid var(--border)",
              borderBottom: spawnOpen ? "1px solid var(--border)" : "1px solid var(--border)",
          ...
          {spawnOpen && (
            <div style={{ ...CARD, borderRadius: "0 0 10px 10px", borderTop: "none" }}>
              <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: 10, color: "var(--text-muted)", display: "block", marginBottom: 4, fontWeight: 500 }}>Model Override (optional)</label>
        ...
        {/* Sub-agents List */}
        <div style={{ marginBottom: 20 }}>
          <span style={SECT}>Active Sub-Agents ({subagents.length})</span>
          {loading ? (
            ...
            <div style={{ ...CARD, padding: "24px 14px", textAlign: "center" }}>
              ...
              <p style={{ fontSize: 11, color: "var(--text-muted)", margin: 0 }}>No active sub-agents</p>
              <p style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 4, opacity: 0.6 }}>Use the Spawn form above to create one</p>
            </div>
          ) : (
                ...
                <div key={sa.id} style={CARD}>
                  <div style={ROW}>
                    ...
                    <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                      ...
                      <button
                        onClick={() => subagentAction(`Kill-${sa.id}`, `/subagents kill ${sa.id}`)}
                      ...
                      <input
                        ...
                        onKeyDown={e => {
                          if (e.key === "Enter" && steerInputs[sa.id]?.trim()) {
                            subagentAction(`Steer-${sa.id}`, `/subagents steer ${sa.id} ${steerInputs[sa.id]}`);
