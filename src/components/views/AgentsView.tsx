...
export function AgentsView() {
  ...
  const saveIdentity = async (id: string) => {
    ...
    try {
      ...
      if (identityName.trim()) cmd += ` --name "${identityName.trim()}"`;
      if (identityEmoji.trim()) cmd += ` --emoji "${identityEmoji.trim()}"`;
      const result = await invoke<{ stdout: string; stderr: string; code: number }>("execute_command", {
        command: cmd,
        cwd: null,
      });
  ...
  const addBinding = async (agentId: string) => {
    ...
    setBinding(true);
    try {
      const flag = bindType === "phone" ? `--phone ${bindValue.trim()}` : `--channel ${bindValue.trim()}`;
      const result = await invoke<{ stdout: string; stderr: string; code: number }>("execute_command", {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      {/* Sidebar */}
      <div style={{ width: 220, flexShrink: 0, borderRight: "1px solid rgba(255,255,255,0.08)", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "14px 16px 10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <h2 style={{ color: "white", fontSize: 15, fontWeight: 600, margin: 0 }}>Agents</h2>
          <div style={{ display: "flex", gap: 4 }}>
            <button
              onClick={() => { loadAgents(); loadBindings(); }}
              style={{ background: "none", border: "none", padding: 4, cursor: "pointer", color: "rgba(255,255,255,0.4)" }}
            >
              <RefreshCw style={{ width: 12, height: 12 }} />
            </button>
            <button
              onClick={() => setShowAddForm(!showAddForm)}
              style={{ background: "rgba(59,130,246,0.2)", border: "none", borderRadius: 6, padding: "4px 8px", color: "#60a5fa", fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}
            >
        ...
        {showAddForm && (
          <div style={{ padding: "0 12px 8px", display: "flex", gap: 4 }}>
            <input
              ...
              style={{ flex: 1, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, padding: "5px 8px", color: "white", fontSize: 11, outline: "none" }}
            />
            <button
              onClick={addAgent}
              disabled={adding}
              style={{ background: "#3B82F6", border: "none", borderRadius: 6, padding: "5px 10px", color: "white", fontSize: 11, cursor: "pointer" }}
            >
        ...
        {error && (
          <div style={{ padding: "0 12px 8px" }}>
            <div style={{ background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 6, padding: "6px 8px", display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 10, color: "#f87171", wordBreak: "break-word" }}>{error}</span>
            </div>
          </div>
        )}
            ...
            agents.map((agent) => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <Bot style={{ width: 16, height: 16, color: agent.isDefault ? "#3B82F6" : "rgba(255,255,255,0.4)", flexShrink: 0 }} />
                  ...
                  {agent.isDefault && (
                    <Star style={{ width: 12, height: 12, color: "#fbbf24", flexShrink: 0 }} />
                  )}
                </div>
              </button>
            ))
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
              <div style={{ width: 48, height: 48, borderRadius: 12, background: "rgba(59,130,246,0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Bot style={{ width: 24, height: 24, color: "#3B82F6" }} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <h3 style={{ margin: 0, color: "white", fontSize: 16, fontWeight: 600 }}>{selected.id}</h3>
                  {selected.isDefault && (
                    <span style={{ fontSize: 9, color: "#fbbf24", background: "rgba(251,191,36,0.15)", padding: "2px 6px", borderRadius: 4, fontWeight: 600 }}>DEFAULT</span>
                  )}
                </div>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: 8, border: "none", background: "rgba(248,113,113,0.15)", color: "#f87171", fontSize: 11, cursor: "pointer" }}
                >
            ...
            {confirmDelete === selected.id && (
              <div style={{ background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 8, padding: 12, marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: "#f87171" }}>Delete agent &ldquo;{selected.id}&rdquo;? This cannot be undone.</span>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <button
                    onClick={() => setConfirmDelete(null)}
                    style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "rgba(255,255,255,0.6)", fontSize: 11, cursor: "pointer" }}
                  >Cancel</button>
                  <button
                    onClick={() => deleteAgent(selected.id)}
                    disabled={deleting}
                    style={{ padding: "4px 10px", borderRadius: 6, border: "none", background: "#ef4444", color: "white", fontSize: 11, cursor: "pointer" }}
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, padding: "6px 8px", color: "white", fontSize: 12, outline: "none", boxSizing: "border-box" }}
                    />
                  </div>
                  <div style={{ width: 80 }}>
                    <label style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", display: "block", marginBottom: 4 }}>Emoji</label>
                    <input
                      value={identityEmoji}
                      onChange={(e) => setIdentityEmoji(e.target.value)}
                      placeholder="🤖"
                      style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, padding: "6px 8px", color: "white", fontSize: 12, outline: "none", textAlign: "center", boxSizing: "border-box" }}
                    />
                  </div>
                  <button
                    onClick={() => saveIdentity(selected.id)}
                    disabled={savingIdentity}
                    style={{ padding: "6px 14px", borderRadius: 6, border: "none", background: "#3B82F6", color: "white", fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" }}
            ...
            {/* Model */}
            <Section title="Model">
              <span style={{ fontSize: 12, fontFamily: "monospace", color: "white", background: "rgba(255,255,255,0.06)", padding: "4px 10px", borderRadius: 6, border: "1px solid rgba(255,255,255,0.1)" }}>
                {selected.model}
              </span>
            </Section>
            ...
            <Section title="Routes">
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {selected.routes.map((route, i) => (
                  <span key={i} style={{ fontSize: 11, color: "rgba(255,255,255,0.7)", background: "rgba(255,255,255,0.04)", padding: "4px 8px", borderRadius: 4, border: "1px solid rgba(255,255,255,0.08)" }}>
                    {route}
                  </span>
                ))}
              </div>
              ...
              {agentBindings(selected.id).length > 0 ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {agentBindings(selected.id).map((b, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,0.04)", padding: "6px 10px", borderRadius: 6, border: "1px solid rgba(255,255,255,0.08)" }}>
                      <div>
                        <span style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", marginRight: 8 }}>{b.type}</span>
              ...
              {showBindForm ? (
                <div style={{ display: "flex", gap: 6, marginTop: 8, alignItems: "center" }}>
                  <select
                    ...
                    style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, padding: "5px 8px", color: "white", fontSize: 11, outline: "none" }}
                  >
                ...
                <button
                  ...
                  style={{ marginTop: 8, background: "rgba(59,130,246,0.15)", border: "none", borderRadius: 6, padding: "5px 10px", color: "#60a5fa", fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}
                >
            ...
            {testResult && (
              <Section title="Test Result">
                <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: 10, fontSize: 12, color: "rgba(255,255,255,0.8)", whiteSpace: "pre-wrap", maxHeight: 200, overflowY: "auto", fontFamily: "monospace" }}>
                  {testResult}
                </div>
              </Section>
            )}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", fontWeight: 500, marginBottom: 6, margin: 0, paddingBottom: 6 }}>{title}</p>
      {children}
    </div>
  );
}
